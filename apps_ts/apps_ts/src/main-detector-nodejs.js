...
let config;
try {
    const content = fs.readFileSync(fs.existsSync(sensorsPath) ? sensorsPath : sensorsFallbackPath, "utf-8");
...
ws.on("message", (data) => {
    try {
        ...
        if (events.length > 0) {
            console.log(`[DETECTOR] Frame ${frameId}: Intrusion detected (${events.length} voxels)`);
            for (const ev of events) {
                console.log(`  voxel ${ev.key}: delta=${ev.delta.toFixed(2)}`);
            }
        }
        else if (frameId % 100 === 0) {
            console.log(`[DETECTOR] Frame ${frameId}: OK (${points.x.length} points)`);
        }
    }
    catch (err) {
...
ws.on("close", () => {
    ...
    console.log(`[DETECTOR] Disconnected after ${duration}s (${frameId} frames processed)`);
    process.exit(0);
});
// クリーンアップ
process.on("SIGINT", () => {
