...
# ================================================================== #
# ハードコード定数 — LiDARマスト高度をセンサー原点を参照基準として設定     #
# ================================================================== #
#
# 座標系: LiDAR センサー原点（z=0）を基準としたセンサーローカル座標
#
#   z = +2.0 m  ... 離着陸面上 +2m (監視上限)
#   z = +0.0 m  ... LiDAR 設置高度 (基準)
#   z = -2.0 m  ... 離着陸面 (LiDARから見て下)
#
# 実測データから確認（PCAP 250808sbir):
#   水平距離: 0.5 ～ 44.8m
#   Z 値: -13.47 ～ +0.04m （LiDAR より下の点が大部分）
#
# ドローン離着陸パッド監視想定:
#   - パッド上方 0～30m: 上限
#   - パッド周辺: 水平距離 ~ 1.8～2.5m
#   - 底面: Z = -2.0m (LiDAR下方, パッド方向)
#   - 上面: Z = +30m (上空)

R_BOTTOM: float = 1.775   # z_bottom での半径 [m]
R_TOP:    float = 2.5     # z_bottom+height での半径 [m]
HEIGHT:   float = 32.0    # 円錐台の高さ [m] (z=-2 から z=+30 までの距離)
Z_BOTTOM: float = -2.0    # 円錐台底面の Z 座標 [m] (パッド方向)
